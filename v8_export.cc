...
#include <vector>
#include <functional>
#include "V8InspectorInterface.h"

///
m_ctx *(*getGoContextFuncEntry)(int);

ValuePtr (*goFunctionCallbackEntry)(int, int, ValuePtr *, int);
///
using namespace v8;
...
void InitV8Go(m_ctx *(*getGoContextFuncIn)(int), ValuePtr (*goFunctionCallbackIn)(int, int, ValuePtr *, int)) {
    getGoContextFuncEntry = getGoContextFuncIn;
    goFunctionCallbackEntry = goFunctionCallbackIn;
}
...
static void FunctionTemplateCallback(const FunctionCallbackInfo<Value> &info) {
    ...
    m_ctx *ctx = getGoContextFuncEntry(ctx_ref);
    ...
    ValuePtr *thisAndArgs = new ValuePtr[args_count + 1];
...
/********** Array ***********/
#define LOCAL_ARRAY(ptr) \
  LOCAL_VALUE(ptr)        \
  Local<Array> array = value.As<Array>()
int GetArrayLen(ValuePtr ptr) {
    LOCAL_ARRAY(ptr);
    return array->Length();
}
